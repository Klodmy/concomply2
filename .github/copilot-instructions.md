...
### Initial Setup
1. Ensure Flask, Flask-SQLAlchemy dependencies installed
2. Set `SECRET_KEY` in `.env` file (loaded by `python-dotenv`)
3. Run `python create_db.py` to create database tables
4. Start app: `python -m flask run` or similar

### Adding Routes
1. Create handler in `app.py` with `@app.route()` decorator
2. Use form data via `request.form.get("field_name")`
3. Query models with SQLAlchemy: `AdminUser.query.filter_by(...)`
4. Use `flash()` for user notifications, render templates with `render_template()`

### Adding Models
1. Define in [models.py](../models.py) with `Mapped` type hints
2. Run `db.create_all()` or recreate database
...
- Flash messages pattern already established (see login.html)

## Important Notes

### Incomplete Features
- `/logout` route is stubbed (needs session clearing)
- [routes.py](../routes.py) and [services.py](../services.py) are empty (consider moving route logic here for modularity)
- No input validation on forms (email format, password strength)
- Dashboard POST handler empty (line 24 in app.py)
